...
select id, brand,model from cars where brand = 'Hyundai';--3
select id , brand , model,price,color from cars where color = 'Red' or color ='Blue';--4
select * from cars where year_of_issue between '2000' and '2010';--5
select  count(brand)from cars group by brand = 'Chevrolet';--6
select avg(year_of_issue)from cars;--7
select  id , model ,brand,year_of_issue from cars where  brand = 'Audi' or brand = 'Toyota' or brand =' Kia' or brand ='Ford';--8
select *from cars where brand ilike'T%';--9
select *from cars where  model ilike '%e';--10
select  * from  cars where brand like '_____';--11
select sum(price) from cars group by brand ='Mercedes-Benz';--12
select min(price) , max(price) from cars;--13
select * from cars where brand <> 'Toyota';--14
select distinct * from cars order by price desc limit 10;--15
select*  from  cars where  year_of_issue between  '2005' and '2015' limit 10;--16
select *from cars where year_of_issue not between '1995' and '2005';--17
select color, count(*) as count from cars group by color order by count desc limit 1;--18
